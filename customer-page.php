...
if (!isset($_SESSION['CusID'])) {
    ...
    $query = $db->prepare("select * from customer,customerfullname where customer.CID=? and customerfullname.CID=?");
    $query->execute([$ID, $ID]);
    ...
    if (isset($_GET['click'])) {
        ...
        for ($i = 0; $i < $count; $i++) {
           ...
           // echo "<script>alert('" . $Gender . "  " . $eachres['Gender'] . "')</script>";
            if ($eachres['Gender'] === $Gender) {
                $SID = '"' . $eachres['SID'] . '"';
            ...
            try {
                ...
                if ($chosenFilter == "saloon") {
                    ...
                    if ($res->rowCount() == 0) {
                        ...
                        for ($i = 0; $i < $res->rowCount(); $i++) {
                            $eachres = $res->fetch(PDO::FETCH_ASSOC);
                            if (strpos($eachres['SalName'], $text) !== false && $eachres['Gender'] === $Gender) {
                                $SID = '"' . $eachres['SID'] . '"';
                                $res2 = $db->query("select * from salAddress where SID = $SID");
                                $address = $res2->fetch(PDO::FETCH_ASSOC);
                                $resultText = $resultText . "<div class='card' onclick='showBookingDialog($SID)'><img src='images/" . $eachres['image'] . "' style='width:100%'/>
                <div class='container'> <p> " . $eachres['SalName'] . " <br> " . $eachres['PhoneNum'] . "<br>" . $address['Street'] . " - " . $address['City']  . "<br>" . $eachres['WebPage'] ."</p>  </div> </div>";
                    ...
                    if ($res->rowCount() == 0) {
                        ...
                        for ($i = 0; $i < $res->rowCount(); $i++) {
                            $eachres = $res->fetch(PDO::FETCH_ASSOC);
                            if (strpos($eachres['City'], $text) !== false && $eachres['Gender'] === $Gender) {
                                $SID = '"' . $eachres['SID'] . '"';
                                $res2 = $db->query("select * from salAddress where SID = $SID");
                                $address = $res2->fetch(PDO::FETCH_ASSOC);
                                $resultText = $resultText . "<div class='card' onclick='showBookingDialog($SID)'><img src='images/" . $eachres['image'] . "' style='width:100%'/>
                <div class='container'> <p> " . $eachres['SalName'] . " <br> " . $eachres['PhoneNum'] . "<br>" . $address['Street'] . " - " . $address['City']  . "<br>" . $eachres['WebPage'] ."</p>  </div> </div>";
                    ...
                    if ($res->rowCount() == 0) {
                        ...
                        for ($i = 0; $i < $res->rowCount(); $i++) {
                            $eachres = $res->fetch(PDO::FETCH_ASSOC);
                            if (strpos($eachres['Service'], $text) !== false && $eachres['Gender'] === $Gender) {
                                $SID = '"' . $eachres['SID'] . '"';
                                $res2 = $db->query("select * from salAddress where SID = $SID");
                                $address = $res2->fetch(PDO::FETCH_ASSOC);
                                $resultText = $resultText . "<div class='card' onclick='showBookingDialog($SID)'><img src='images/" . $eachres['image'] . "' style='width:100%'/>
...
<!--    <h1>أهلاً يا سيد كستومر</h1>-->
    <?php
        if($Gender === "M"){
            echo "<h1>أهلا يا سيد $FullName</h1>";
        }else {
            echo "<h1>أهلا يا سيدة $FullName</h1>";
        }
    ?>
</div>
<div class="header">
...
<div id="dlgbox" class="dlgbox">
    ...
    <div id="dlg-footer" class="dlg-footer">
        <button onclick="dlgOK()">إغلاق</button>
    </div>
</div>
...
<!--    <div class="dlg-footer">-->
<!--        <button class="delete_account" onclick="dlgOK5('--><?php //echo $ID; ?>
<!--//')">حذف حسابي-->
<!--</button>-->
